    ...
    <div class="col-12">
        <div class="row">
            <div class="col-12">
                ...
                <select onChange="window.location.href=this.value" id="inputState" class="form-control" name="tanggal_pre_order">
                    ...
                    <option value="{{url('Pembeli/Transaksi/Tipe/'.$pengiriman['status'])}}" selected>{{$pengiriman['status2']}}</option>    
                    @else
                    <option value="{{url('Pembeli/Transaksi/Tipe/'.$pengiriman['status'])}}">{{$pengiriman['status2']}}</option>
                    @endif
                    @endforeach
                </select>
            </div>
        ...
        <div class="card">
            <div class="card-body" style="overflow-x:auto;">
                <h5 class="card-title"><strong>Transaksi {{$data['tipe2']}}</strong></h5>
                        ...
                        @foreach ($data['transaksi'] as $key => $transaksi)
                            <tr>
                                ...
                                <td>
                                    @if($data['tipe']=="BelumDibayar")
                                        <a class="badge badge-primary mb-1 text-white" data-toggle="modal" data-target=".detail-transaksi" onclick="lihatItem('{{$transaksi->id}}')">Detail</a>
                                    ...
                                    @elseif($data['tipe']=="Selesai")
                                        ...
                                        <a class="badge badge-primary mb-1 text-white" href="{{ url('Pembeli/Transaksi/Konfirmasi/'.$transaksi->id) }}">Konfirmasi</a>
                                        @endif
                                    @else
                                        <a class="badge badge-primary mb-1 text-white" data-toggle="modal" data-target=".detail-transaksi" onclick="lihatItem('{{$transaksi->id}}')">Detail</a>
                                    @endif
                            ...
                            <div class="modal fade" id="modal{{$transaksi->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form method="POST" action="{{ url('Pembeli/Transaksi/Bayar/'.$transaksi->id) }}" enctype='multipart/form-data'>
                                        @csrf
...
<script>
    function lihatItem(transaksi)
    {
        ...
        $.ajax({
            type: 'GET',
            url: '/Pembeli/Transaksi/Detail/'.concat(transaksi),
