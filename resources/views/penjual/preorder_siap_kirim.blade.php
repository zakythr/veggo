        ...
        <div class="col-md-12">
            <div class="card">
                ...
                @csrf
                    ...
                    <div class="row p-3">
                        <div class="col-8">
                            <h1 class="card-title"><strong>Daftar Siap Kirim | {{Carbon\Carbon::parse($data['filter_tanggal'])->format('D, d M Y')}}</strong></h1>
                        </div>
                        <div class="col-2">
                            <select name="tanggal" class="form-control" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                                ...
                                @foreach ($data['tanggal'] as $tanggal)
                                    ...
                                    @else            
                                        <option value="{{url('/Penjual/PreOrder/SiapKirim/Tanggal/'.$tanggal->tanggal_pre_order)}}" >{{Carbon\Carbon::parse($tanggal->tanggal_pre_order)->format('D, d M Y')}} ({{$tanggal->total}})</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        <div class="col-2">
                            <a class="btn btn-sm btn-primary" href="{{url('Penjual/Pengiriman/Tambah')}}">Lakukan Pengiriman</a>
                        </div>
                </div>
                </form>
                            ...
                            <tbody>
                                @foreach ($data['preorder'] as $key => $preorder)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="badge badge-secondary text-white mb-1">Siap Kirim</a>
