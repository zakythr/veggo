        ...
        <div class="col-md-12">
            <div class="card">
                ...
                @csrf
                    ...
                    <div class="row p-3">
                        <div class="col-7">
                            <h1 class="card-title"><strong>Daftar Proses Order | {{Carbon\Carbon::parse($data['filter_tanggal'])->format('D, d M Y')}}</strong></h1>
                        </div>
                        <div class="col-5">
                            <select name="tanggal" class="form-control" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                                <option selected disabled>Tanggal Proses Order</option>
                                @foreach ($data['tanggal'] as $tanggal)
                                    ...
                                    @else            
                                        <option value="{{url('/Penjual/PreOrder/Proses/Tanggal/'.$tanggal->tanggal_pre_order)}}" >{{Carbon\Carbon::parse($tanggal->tanggal_pre_order)->format('D, d M Y')}} ({{$tanggal->total}})</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                            ...
                            <tbody>
                                @foreach ($data['preorder'] as $key => $preorder)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="badge badge-info mb-1" href="{{url('/Penjual/Pengiriman/Finalisasi/'.$preorder->id)}}">Order Terkirim</a>
