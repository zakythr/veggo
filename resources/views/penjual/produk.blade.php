        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row p-3">
                        <div class="col-6">
                            <h1 class="card-title"><strong>Data Produk</strong></h1>
                        </div>
                        <div class="col-6">
                            <a id="btnGroupDrop1" type="button" class="btn btn-success btn-sm float-right" href="{{url('/Penjual/Produk/Tambah')}}">
                                Tambah
                            </a>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach ($produks as $key => $produk)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>
                                            <a href="{{url('/Penjual/Produk/Ubah/'.$produk->id)}}">{{$produk->kode}} | {{$produk->nama}}</a> 
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{url('/Penjual/Produk/Hapus')}}" method="POST">
                                                @csrf
                                                <input type="hidden" name="id" value="{{$produk->id}}">
                                                <button type="submit" class="btn btn-xs btn-danger m-1" onclick="return confirm('Apakah anda yakin ingin menghapus ?');">Hapus</button>
                                            </form>   
                                            {{-- @endif --}}
                                        </td>
